...
def main(args):
    ...
    def compute_loss_pi(batch):
        ...
        # get probability and log-probability of action given observation \pi(a|s)
        ...
        if args.loss_mode == 'vpg':
            ...
            r_t = torch.exp(logp - logp_old)
            loss_pi = -torch.mean(torch.min(
                r_t*psi,
                torch.clamp(r_t, 1-args.clip_ratio, 1+args.clip_ratio) * psi
            ))
        else:
    ...
    def compute_loss_v(batch):
        ...
        loss_v = torch.mean((v - ret.unsqueeze(1))**2) # TODO: fix this
        return loss_v
    ...
    for epoch in range(args.epochs):
        ...
        for t in range(steps_per_epoch):
            # this step has no gradient, only used to collect data
            # a -> action, v -> value, logp -> log probability of action
            obs = torch.as_tensor(o, dtype=torch.float32).to(args.device)
            a, v, logp = ac.step(obs)

            # using environment, take a step with the action
            next_o, r, d, truncated, _ = env.step(a)
            ep_ret += r # update return for epoch
            ep_len += 1 # update length for epoch
            ...
            epoch_ended = t==steps_per_epoch-1 # at the final step of a trajectory

            if terminal or epoch_ended:
                # if trajectory didn't reach terminal state, bootstrap value target
                if timeout or epoch_ended:
                    # value is non-zero at timeout or when epoch ends
                    _, v, _ = ac.step(torch.as_tensor(o, dtype=torch.float32).to(args.device))
                else:
                    v = 0 # value is 0 at terminal state
                buf.finish_path(v) # compute advantage estimates and rewards-to-go
                if terminal:
        ...
        # Perform Vanilla Policy Gradient (VPG) update!
        update()
